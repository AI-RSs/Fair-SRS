...
def LT_inclusion(scores_ind,alpha,df_t,lt,scores500_ind):   #df_t is the dataframe for test sequences (refer to preprocess.py)
    alpha=0.5  #hyper parameter to determine how many LT is needed based on the LID in current session
    ...
    for ix in range(len(scores_ind)):
        ...
        pos2=[ij for ij, x in enumerate(pd.Series(scores500_ind[ix]).isin(lt)) if x]
        #print('position of LT items in top-500:',pos2)
        a=[]
        for p in range(len(pos2)):
            a.append(scores500_ind[ix][pos2[p]])
